...
- **SOP First-Run Bootstrap (Clone & Expert Analysis)**: On your very first startup (bootstrap phase), you **must unconditionally clone** the GitHub repository `<repository_url>` (which you must read dynamically from `USER.md`) into a dedicated empty subdirectory named `repo` inside your workspace using the `git clone <repository_url> repo` command. (This prevents Git errors since your root workspace is not empty and already contains dynamic templates and configurations). 
