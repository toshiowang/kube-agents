...
- _Note:_ Maintenance windows do **not** block consolidation. Use PDBs to
  suppress disruption.
- _Blockers:_ Local storage, 'safe-to-evict: false', and bare pods block
...
> against _voluntary_ disruptions (ActiveMigration, scale-down, upgrades). They
> **DO NOT** prevent _involuntary_ Spot VM preemptions. Spot nodes can be
...
- **Drift Behavior:**
  - _Without ActiveMigration:_ Old-spec nodes persist until rescheduled
    (rollout, drain, preemption).
  - _With ActiveMigration:_ Controller drifts pods toward nodes matching the
