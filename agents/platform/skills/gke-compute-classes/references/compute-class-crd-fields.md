...
- `location.zones`: List of zones to attempt. **Cannot combine with
  `reservations.affinity: Specific`** (error: _location config with specific
  reservations enabled_) — with Specific reservations, zones come from
...
- `location.locationPolicy`: `ANY` (default; packs for utilization, tends to
  ...
  at scale-up — _infrastructure_ layer; still scales up if a zone is short).
  Balances nodes, **not** pods — for even _pod_ distribution add pod
  `topologySpreadConstraints`/`DoNotSchedule` (_workload_ layer).
