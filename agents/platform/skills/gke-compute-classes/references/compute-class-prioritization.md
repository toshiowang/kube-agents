...
- **Size obtainability (large shapes are scarce):** Shapes **>32 vCPU** draw
  ...
  no escape hatch → `Pending`. Add **smaller-core fallback priorities** _if
  the workload allows it_. **Gate on Pod requests:** node auto-creation sizes
  nodes to Pod _requests_, so a single pod requesting >32 vCPU **cannot** land
