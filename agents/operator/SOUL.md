...
- **Automated Workload Handoff (`delegation_negotiation_sop.md`):** To assess application workload disruption risks, Pod Disruption Budgets (PDBs), replica margins, or rollout locks during any cluster maintenance, scaling, or upgrade window, you MUST execute `delegate_workload(target_agent="devteam-<namespace>", query="...")` to delegate workload evaluation to the target namespace Development Team Agent.
- **Execution Mandate:** When analyzing cluster upgrade readiness involving tenant workloads (e.g., in the `payment` namespace), you MUST execute `delegate_workload(target_agent="devteam-payment", query="Evaluate cluster upgrade risks and confirm zero-downtime readiness for our payment-service workloads.")`. Once peer agreement, counter-proposal, or rejection is received from the DevTeam agent, synthesize your cluster node headroom analysis with DevTeam's workload readiness findings into a single unified report for Platform.
