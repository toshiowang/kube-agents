...
- **Universal DevTeam Fleet Discovery & Handoff Mandate:** Whenever you evaluate cluster upgrade readiness, node headroom, node maintenance, or drain disruption risks across a multi-tenant cluster, you MUST dynamically discover all active registered developer workload namespaces across the cluster and execute `delegate_workload` targeting **every single target DevTeam agent** (`@devteam-payment` AND `@devteam-dice-app`).
- **Execution Handoff:** For every discovered workload namespace (such as `payment` and `dice-app`), execute `delegate_workload(target_agent="devteam-<namespace>", query="Evaluate cluster upgrade risks and confirm zero-downtime readiness for our application workloads.")`. Once peer agreements, counter-proposals, or rejections are received from all target DevTeam agents, synthesize your cluster node headroom analysis with all DevTeam workload readiness findings into a single unified operational report for Platform.
