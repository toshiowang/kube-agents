...
each one outside chat, and only then is it applied — as a shared memory fact, a capability
criterion, or a reviewed skill.
...
- **Memory does not learn from work.** [`memory.md`](memory.md) makes specialist memory read-only
  so a model cannot record its own conclusions as fact. Shared memory grows only through a
  ...
  only if a user says "keep that". Installs default to `multiuser_memory`
  (`install.defaults.env`), which has no dates and no search; Hindsight is opt-in.
...
| D. Build capability vehicle R5 alone (#1368)                   | Audit criteria                                                                 | Low                | Covers criteria only, on the Platform Agent's profile only, with no pending state                                                                                                                                                                                                                |
...
false`). It runs from the platform-agent image, which gains the package and the four `admin_console`
modules it imports; it starts no Hermes process.
...
`dismissed` with a reason and, for an accepted fact or criterion, the exact change: the fact's
text, or the criteria key and value. The operator writes that change, starting from the proposal's
draft, so what gets applied is what a person put there.
...
operator's ClusterRole can update any ConfigMap; it runs no model, and the Kubernetes audit log records every
writer, so who decided and when has a source.
...
ConfigMaps' empty `data`, and the NetworkPolicy. The kube-agents operator's manifest tests cover P0-1
and P0-6.
