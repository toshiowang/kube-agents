...
| `docs/designs/gchat-markdown-rendering.md` | Feature design | Which Markdown constructs Google Chat renders badly and what the image patch does about them: unanchored space-collapse flattening nested lists, and pipe tables redrawn as a padded fenced grid or per-row stanzas. | `format_message` call ordering, `_ph` protection, `MAX_TABLE_WIDTH` fallback, layer budget | Design of record; implemented |
